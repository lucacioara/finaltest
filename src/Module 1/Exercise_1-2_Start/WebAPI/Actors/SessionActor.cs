...
namespace GameAPI.Actors
{
    public class SessionActor : Actor, ISessionActor
    {
        ...
        public async Task CastInVote(string playerID, Choice choice)
        {
        ...
        {
            await StateManager.SetStateAsync(SessionName, new Session { Player1 = new Player() { Id = playerID }, State = SessionState.Created });
        }

        public async Task EnterExistingMatch(string playerID)
        {
            var session = await StateManager.GetStateAsync<Session>(SessionName);
            session.Player2 = new Player() { Id = playerID };
